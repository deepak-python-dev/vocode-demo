...
const Conversation = ({
  ...
  const [voice, setVoice] = React.useState<string>("synthesizer_eleven_labs");
  const [voices, setVoices] = React.useState<Array<any>>([]);
        ...
        <HStack width="96%" position="absolute" top={"10%"} left="2%">
          ...
          <Select
            ...
            onChange={(event) => {
              ...
              config.synthesizerConfig = synthesizerConfig;
              console.log(config.synthesizerConfig);
              event.target.value && setVoice(event.target.value);
            }}
